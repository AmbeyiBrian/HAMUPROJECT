...
class ExpensesViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.user_class == 'Director':
            ...
            queryset = Expenses.objects.all().select_related('shop')
        else:
            # Agents only see expenses from their shop
            queryset = Expenses.objects.filter(shop=user.shop).select_related('shop')
              # Handle date range filtering with start_date and end_date parameters
        start_date = self.request.query_params.get('start_date')
        end_date = self.request.query_params.get('end_date')
        
        # Log received date parameters for debugging
        print(f"DEBUG: Received date filters - start_date: {start_date}, end_date: {end_date}")
        
        if start_date:
            queryset = queryset.filter(created_at__gte=start_date)
            print(f"DEBUG: Filtered by start_date: {start_date}, count: {queryset.count()}")
        if end_date:
            queryset = queryset.filter(created_at__lte=end_date)
            print(f"DEBUG: Filtered by end_date: {end_date}, count: {queryset.count()}")
