...
class MeterReadingViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['shop', 'reading_type', 'reading_date']
    def get_queryset(self):
        ...
        if user.user_class == 'Director':
            ...
            queryset = MeterReading.objects.all().select_related('shop')
        else:
            # Agents only see meter readings from their shop
            queryset = MeterReading.objects.filter(shop=user.shop).select_related('shop')
            
        # Apply date range filtering if provided in query parameters
    ...
    def consumption_report(self, request):
        ...
        """
        queryset = self.get_queryset()  # This now includes date filtering from the get_queryset method
