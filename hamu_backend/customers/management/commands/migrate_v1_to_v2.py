...
class Command(BaseCommand):
    ...
    def migrate_meter_readings(self):
        ...
        try:
            ...
            failure_count = 0
            skipped_count = 0
              # Write detailed pre-migration analysis to log file            logger.info("====== PRE-MIGRATION ANALYSIS =====")
            shop_reading_counts = {}
            date_distribution = {}
            year_distribution = {}# Debug the actual fields in the first record
            if old_readings:
                first_reading = old_readings[0]
                logger.info(f"Sample reading fields: {list(first_reading.keys())}")
                logger.info(f"Sample date field value: {first_reading.get('date', 'N/A')}, type: {type(first_reading.get('date', None))}")
                logger.info(f"Sample time field value: {first_reading.get('time', 'N/A')}, type: {type(first_reading.get('time', None))}")
                
                # Show a few sample readings with their date/time values for verification
                for i, reading in enumerate(old_readings[:5]):  # Show first 5 readings
                    logger.info(f"Sample reading {i+1}: ID={reading['id']}, " +
                               f"date={reading.get('date', 'N/A')}, " +
                               f"time={reading.get('time', 'N/A')}, " +
                               f"type={reading.get('reading_type', 'N/A')}")
                for reading in old_readings:
                    ...
                    try:
                        ...
                        # In case reading_date doesn't have a year attribute
                        logger.warning(f"Could not extract year from date {reading_date} for reading ID {reading['id']}")
            logger.info(f"Readings by shop: {shop_reading_counts}")
            
            # Log year distribution in a more readable format
            logger.info("Readings by year:")
            for year, count in sorted(year_distribution.items()):
                logger.info(f"  {year}: {count} records")
                
            # Log detailed date distribution if needed for debugging
                ...
                try:
                    with transaction.atomic():
                        ...
                        if old_reading['id'] in existing_ids:
                            ...
                            skipped_count += 1
                            continue
                          # Get date and time from separate fields in the old database
                        shop_id = old_reading['shop_id']
                        reading_type = old_reading.get('reading_type', 'unknown')
                          # Get date and time from their respective fields - should always exist in source database
                        reading_date = old_reading['date']
                        reading_time = old_reading['time']
                        ...
                        if constraint_key in existing_constraints:
                            ...
                            for offset in range(-15, 16):
                                ...
                                if new_key not in existing_constraints:
                                    original_date = old_reading['date']
                                    reading_date = new_date
                                    logger.info(f"Modified reading date from {original_date} to {reading_date} (offset {offset} days) to avoid unique constraint violation")
                            ...
                            if not duplicate_found:
                                ...
                                if new_key not in existing_constraints:
                                    ...
                                    failure_count += 1
                                    continue
                          # Override auto_now_add fields by directly creating the model without auto_now_add                        # This requires a specialized approach to bypass the auto_now_add constraint
                        reading = MeterReading(
                            id=old_reading['id'],
                            shop_id=shop_id,
                            agent_name=old_reading['agent_name'],
                            value=float(old_reading['value']),
                        ...
                        logger.info(f"Migrated meter reading ID {reading.id} with date: {reading_date}, time: {reading_time}")
                        
                        # Track for year summary
                        year = reading_date.year
                        if not hasattr(self, 'migrated_year_counts'):
                            self.migrated_year_counts = {}
                        if year not in self.migrated_year_counts:
                            self.migrated_year_counts[year] = 0
                        self.migrated_year_counts[year] += 1
            ...
            # Create detailed summary of migrated records by year
            logger.info(f"Meter readings migration summary: {success_count} migrated, {skipped_count} skipped, {failure_count} failed")
            
            # Log summary of migrated readings by year
            if hasattr(self, 'migrated_year_counts') and self.migrated_year_counts:
                logger.info("Successfully migrated readings by year:")
                for year, count in sorted(self.migrated_year_counts.items()):
                    logger.info(f"  {year}: {count} records")
            else:
                logger.info("No year distribution available for migrated readings")
